...
async function fetchJsonPreferNonEmptyMulti(cookieRef, urls, label, rowCount) {
  ...
  for (let i = 0; i < urls.length; i++) {
    try {
      const resp = await (0, import_obsidian2.requestUrl)({ url: urls[i], method: "GET", headers: buildWebGetHeaders(cookieRef.value) });
      applyResponseCookies(cookieRef, resp);
...
async function fetchReviews(cookieRef, bookId) {
  const bid = encodeURIComponent(bookId);
  const q = `bookId=${bid}&listType=1&mine=1&syncKey=0`;
...
function extractWereadReviewPayloads(json) {
  ...
  for (const wrap of reviews) {
    ...
    out.push({
      ...
      realBookId: rev.bookId ? String(rev.bookId) : void 0,
    });
  }
  return out;
}
function reviewNoteText(review) {
  if (review.type === 4) {
    return review.content ? stripHtml(String(review.content)).trim() || void 0 : void 0;
  }
...
function highlightsFromReviews(bookId, json, bookmarkRanges) {
  ...
  for (const rev of reviews) {
    if (rev.realBookId && rev.realBookId !== bookId) continue;
    ...
    if (!content) continue;
    const note = thought && (thought !== content || rev.type === 4) ? thought : void 0;
    out.push({
...
function shouldSyncBook(row, existing, forceFull) {
  ...
  const nextBookmarkCount = (_c = row.bookmarkCount) != null ? _c : 0;
  if (((_d = existing.notebookNoteCount) != null ? _d : -1) !== nextNoteCount) return true;
  if (((_e = existing.notebookReviewCount) != null ? _e : -1) !== nextReviewCount) return true;
  if (((existing.notebookBookmarkCount) != null ? existing.notebookBookmarkCount : -1) !== nextBookmarkCount) return true;
  return false;
}
