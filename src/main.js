...
var HighlightPanelView = class extends import_obsidian5.ItemView {
  ...
  renderGraphSection(container, book, scopeBook) {
    ...
    const draw = () => {
      ...
      for (const edge of edges) {
        ...
        const color = isConnected
          ? (_a2 = HINT_COLORS[edge.hint]) != null
            ? _a2
            : "#6366f1"
          : edge.dim
            ? "#8b5cf6"
    ...
    canvas.addEventListener("click", (e) => {
      ...
      activeNode =
        (activeNode == null ? void 0 : activeNode.id) === (clicked == null ? void 0 : clicked.id) ? null : clicked;
      draw();
    });
  ...
  openMindMapExpanded(scopeBook) {
    ...
    zoomOutBtn.addEventListener("click", () => {
      ...
      wrap?.readflowMmControls?.zoomOut();
    });
    zoomInBtn.addEventListener("click", () => {
      const wrap = contentEl.querySelector(".readflow-mm-wrap");
      wrap?.readflowMmControls?.zoomIn();
    });
    fitBtn.addEventListener("click", () => {
      const wrap = contentEl.querySelector(".readflow-mm-wrap");
      wrap?.readflowMmControls?.resetView();
    });
    requestAnimationFrame(function () {
      modal.open();
      requestAnimationFrame(function () {
