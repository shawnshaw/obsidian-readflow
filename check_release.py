...
def main() -> int:
    ...
    if package.get("version") != manifest["version"]:
        fail("package.json version does not match manifest.json version")
    ok("package.json version matches manifest.json")

    changelog = (ROOT / "CHANGELOG.md").read_text(encoding="utf-8")
    current_version = manifest["version"]
    if f"## {current_version}" not in changelog:
        fail(f"CHANGELOG.md missing section for current version {current_version}")
    ok(f"CHANGELOG.md documents version {current_version}")

    if manifest["version"] not in versions:
        fail(f"Current manifest version {current_version} is not present in versions.json")
    ...
    main_js = (ROOT / "main.js").read_text(encoding="utf-8", errors="replace")
    if "GENERATED FILE" not in main_js[:800] and "DO NOT EDIT" not in main_js[:800]:
        warn("main.js banner missing GENERATED FILE / DO NOT EDIT reminder")
    else:
        ok("main.js contains do-not-edit banner")

    src_main = (ROOT / "src/main.js").read_text(encoding="utf-8", errors="replace")
    if len(src_main) < 1000:
        fail("src/main.js looks too small to be the canonical source")
    ok("src/main.js size looks valid")
